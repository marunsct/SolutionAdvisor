sap.ui.define([
  ...
  return Controller.extend("sd.solutionadvisor.controller.AdminCleanCoreLevels", {
    ...
    onInit: function() {
      ...
      oRouter.getRoute("AdminCleanCoreLevels").attachPatternMatched(this._onRouteMatched, this);
    },
    ...
    _onRouteMatched: function() {
      ...
      const oTable = this.byId("levelsTable");
      if (oTable) {
    ...
    _loadData: function() {
      ...
      const fnUpdateCount = () => {
        ...
        if (iCount > 0) {
          MessageToast.show(`Loaded ${iCount} level records`);
        }
    ...
    onCreate: function() {
      MessageBox.information("Create level dialog - to be implemented");
    },
    ...
    _openEditDialog: function(oContext, bReadOnly) {
      const oModel = this.getView().getModel("admin");
      const sSelect = [
        "ID","level","levelName","description","technicalDebtMultiplier","cloudReadinessFactor","upgradeImpactMultiplier","createdAt","createdBy","modifiedAt","modifiedBy"
      ].join(",");
      ...
      const openDialog = (d) => {
        ...
        const oEditModel = new JSONModel(Object.assign({}, oData, {
          editable: !bReadOnly,
          dialogTitle: bReadOnly ? "View Clean Core Level" : "Edit Clean Core Level"
        }));
        this._oEditContext = oContext;
        if (!this._oEditDialog) {
          Fragment.load({
            id: this.getView().getId(),
            name: "sd.solutionadvisor.view.fragments.EditCleanCoreLevelDialog",
            controller: this
          }).then((oDialog) => {
    ...
    onEditConfirm: function() {
      ...
      if (!d.levelName || !d.description) {
        MessageBox.error("Please fill in required fields (Name, Description)");
        return;
      }
      ["levelName","description","technicalDebtMultiplier","cloudReadinessFactor","upgradeImpactMultiplier"].forEach((f)=>{
    ...
    onDelete: function() {
      MessageBox.warning("Delete is disabled for Clean Core Levels as they are system-critical master data.");
    },
        ...
        const aFilters = [
          new Filter({
            filters: [
              ...
              new Filter("levelName", FilterOperator.Contains, sQuery),
